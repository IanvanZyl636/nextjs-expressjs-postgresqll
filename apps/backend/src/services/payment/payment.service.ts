...
export async function createPaymentForOrder(order: OrderGetPayload<{ include: { orderItems: true } }>, provider: PaymentProvider = PaymentProvider.PAYFAST) {    
...
export async function handleProviderNotification(provider: PaymentProvider, payload: Record<string, any>) {
    const providerImpl = getProvider(provider);
    if (!providerImpl) throw new HttpError(500, 'Payment provider not implemented');
