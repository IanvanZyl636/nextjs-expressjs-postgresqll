import { F1Service } from './f1.service';
import { prisma } from '../../integrations/prisma';
import { ErgastService } from '../../integrations/ergast/ergast.service';

describe('getChampionBySeasons', () => {
  it('run function', async () => {
    try {
      const mockData = {
        MRData: {
          xmlns: 'http://ergast.com/mrd/1.5',
          series: 'f1',
          url: 'http://ergast.com/api/f1/driverstandings/1.json',
          limit: '1',
          offset: '63',
          total: '75',
          StandingsTable: {
            driverStandings: '1',
            StandingsLists: [
              {
                season: '2013',
                round: '19',
                DriverStandings: [
                  {
                    position: '1',
                    positionText: '1',
                    points: '397',
                    wins: '13',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructors: [
                      {
                        constructorId: 'red_bull',
                        url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                        name: 'Red Bull',
                        nationality: 'Austrian',
                      },
                    ],
                  },
                ],
              },
            ],
          },
        },
      };
      jest.spyOn(ErgastService, 'getDriverStandings').mockResolvedValue(mockData);

      const resp = (await Promise.all([
        F1Service.getChampionBySeasons(2013),
        F1Service.getChampionBySeasons(2013),
        F1Service.getChampionBySeasons(2013),
        F1Service.getChampionBySeasons(2013),
      ]))[0][0];

      expect(resp.year).toBe(Number(mockData.MRData.series));
    } catch (e) {
      console.error(e);
    }
  });
});

describe('getRaceWinnersBySeason', () => {
  it('run function', async () => {
    try {
      const mockData = {
        MRData: {
          xmlns: 'http://ergast.com/mrd/1.5',
          series: 'f1',
          url: 'http://ergast.com/api/f1/2013/results/1.json',
          limit: '30',
          offset: '0',
          total: '19',
          RaceTable: {
            season: '2013',
            position: '1',
            Races: [
              {
                season: '2013',
                round: '1',
                url: 'http://en.wikipedia.org/wiki/2013_Australian_Grand_Prix',
                raceName: 'Australian Grand Prix',
                Circuit: {
                  circuitId: 'albert_park',
                  url: 'http://en.wikipedia.org/wiki/Melbourne_Grand_Prix_Circuit',
                  circuitName: 'Albert Park Grand Prix Circuit',
                  Location: {
                    lat: '-37.8497',
                    long: '144.968',
                    locality: 'Melbourne',
                    country: 'Australia',
                  },
                },
                date: '2013-03-17',
                time: '06:00:00Z',
                Results: [
                  {
                    number: '7',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'raikkonen',
                      permanentNumber: '7',
                      code: 'RAI',
                      url: 'http://en.wikipedia.org/wiki/Kimi_R%C3%A4ikk%C3%B6nen',
                      givenName: 'Kimi',
                      familyName: 'Räikkönen',
                      dateOfBirth: '1979-10-17',
                      nationality: 'Finnish',
                    },
                    Constructor: {
                      constructorId: 'lotus_f1',
                      url: 'http://en.wikipedia.org/wiki/Lotus_F1',
                      name: 'Lotus F1',
                      nationality: 'British',
                    },
                    grid: '7',
                    laps: '58',
                    status: 'Finished',
                    Time: { millis: '5403225', time: '1:30:03.225' },
                    FastestLap: {
                      rank: '1',
                      lap: '56',
                      Time: { time: '1:29.274' },
                      AverageSpeed: { units: 'kph', speed: '213.845' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '2',
                url: 'http://en.wikipedia.org/wiki/2013_Malaysian_Grand_Prix',
                raceName: 'Malaysian Grand Prix',
                Circuit: {
                  circuitId: 'sepang',
                  url: 'http://en.wikipedia.org/wiki/Sepang_International_Circuit',
                  circuitName: 'Sepang International Circuit',
                  Location: {
                    lat: '2.76083',
                    long: '101.738',
                    locality: 'Kuala Lumpur',
                    country: 'Malaysia',
                  },
                },
                date: '2013-03-24',
                time: '08:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '56',
                    status: 'Finished',
                    Time: { millis: '5936681', time: '1:38:56.681' },
                    FastestLap: {
                      rank: '3',
                      lap: '45',
                      Time: { time: '1:40.446' },
                      AverageSpeed: { units: 'kph', speed: '198.661' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '3',
                url: 'http://en.wikipedia.org/wiki/2013_Chinese_Grand_Prix',
                raceName: 'Chinese Grand Prix',
                Circuit: {
                  circuitId: 'shanghai',
                  url: 'http://en.wikipedia.org/wiki/Shanghai_International_Circuit',
                  circuitName: 'Shanghai International Circuit',
                  Location: {
                    lat: '31.3389',
                    long: '121.22',
                    locality: 'Shanghai',
                    country: 'China',
                  },
                },
                date: '2013-04-14',
                time: '07:00:00Z',
                Results: [
                  {
                    number: '3',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'alonso',
                      permanentNumber: '14',
                      code: 'ALO',
                      url: 'http://en.wikipedia.org/wiki/Fernando_Alonso',
                      givenName: 'Fernando',
                      familyName: 'Alonso',
                      dateOfBirth: '1981-07-29',
                      nationality: 'Spanish',
                    },
                    Constructor: {
                      constructorId: 'ferrari',
                      url: 'http://en.wikipedia.org/wiki/Scuderia_Ferrari',
                      name: 'Ferrari',
                      nationality: 'Italian',
                    },
                    grid: '3',
                    laps: '56',
                    status: 'Finished',
                    Time: { millis: '5786945', time: '1:36:26.945' },
                    FastestLap: {
                      rank: '4',
                      lap: '46',
                      Time: { time: '1:39.506' },
                      AverageSpeed: { units: 'kph', speed: '197.210' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '4',
                url: 'http://en.wikipedia.org/wiki/2013_Bahrain_Grand_Prix',
                raceName: 'Bahrain Grand Prix',
                Circuit: {
                  circuitId: 'bahrain',
                  url: 'http://en.wikipedia.org/wiki/Bahrain_International_Circuit',
                  circuitName: 'Bahrain International Circuit',
                  Location: {
                    lat: '26.0325',
                    long: '50.5106',
                    locality: 'Sakhir',
                    country: 'Bahrain',
                  },
                },
                date: '2013-04-21',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '2',
                    laps: '57',
                    status: 'Finished',
                    Time: { millis: '5760498', time: '1:36:00.498' },
                    FastestLap: {
                      rank: '1',
                      lap: '55',
                      Time: { time: '1:36.961' },
                      AverageSpeed: { units: 'kph', speed: '200.938' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '5',
                url: 'http://en.wikipedia.org/wiki/2013_Spanish_Grand_Prix',
                raceName: 'Spanish Grand Prix',
                Circuit: {
                  circuitId: 'catalunya',
                  url: 'http://en.wikipedia.org/wiki/Circuit_de_Barcelona-Catalunya',
                  circuitName: 'Circuit de Barcelona-Catalunya',
                  Location: {
                    lat: '41.57',
                    long: '2.26111',
                    locality: 'Montmeló',
                    country: 'Spain',
                  },
                },
                date: '2013-05-12',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '3',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'alonso',
                      permanentNumber: '14',
                      code: 'ALO',
                      url: 'http://en.wikipedia.org/wiki/Fernando_Alonso',
                      givenName: 'Fernando',
                      familyName: 'Alonso',
                      dateOfBirth: '1981-07-29',
                      nationality: 'Spanish',
                    },
                    Constructor: {
                      constructorId: 'ferrari',
                      url: 'http://en.wikipedia.org/wiki/Scuderia_Ferrari',
                      name: 'Ferrari',
                      nationality: 'Italian',
                    },
                    grid: '5',
                    laps: '66',
                    status: 'Finished',
                    Time: { millis: '5956596', time: '1:39:16.596' },
                    FastestLap: {
                      rank: '5',
                      lap: '53',
                      Time: { time: '1:26.681' },
                      AverageSpeed: { units: 'kph', speed: '193.329' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '6',
                url: 'http://en.wikipedia.org/wiki/2013_Monaco_Grand_Prix',
                raceName: 'Monaco Grand Prix',
                Circuit: {
                  circuitId: 'monaco',
                  url: 'http://en.wikipedia.org/wiki/Circuit_de_Monaco',
                  circuitName: 'Circuit de Monaco',
                  Location: {
                    lat: '43.7347',
                    long: '7.42056',
                    locality: 'Monte-Carlo',
                    country: 'Monaco',
                  },
                },
                date: '2013-05-26',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '9',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'rosberg',
                      permanentNumber: '6',
                      code: 'ROS',
                      url: 'http://en.wikipedia.org/wiki/Nico_Rosberg',
                      givenName: 'Nico',
                      familyName: 'Rosberg',
                      dateOfBirth: '1985-06-27',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'mercedes',
                      url: 'http://en.wikipedia.org/wiki/Mercedes-Benz_in_Formula_One',
                      name: 'Mercedes',
                      nationality: 'German',
                    },
                    grid: '1',
                    laps: '78',
                    status: 'Finished',
                    Time: { millis: '8272056', time: '2:17:52.056' },
                    FastestLap: {
                      rank: '6',
                      lap: '74',
                      Time: { time: '1:18.327' },
                      AverageSpeed: { units: 'kph', speed: '153.510' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '7',
                url: 'http://en.wikipedia.org/wiki/2013_Canadian_Grand_Prix',
                raceName: 'Canadian Grand Prix',
                Circuit: {
                  circuitId: 'villeneuve',
                  url: 'http://en.wikipedia.org/wiki/Circuit_Gilles_Villeneuve',
                  circuitName: 'Circuit Gilles Villeneuve',
                  Location: {
                    lat: '45.5',
                    long: '-73.5228',
                    locality: 'Montreal',
                    country: 'Canada',
                  },
                },
                date: '2013-06-09',
                time: '18:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '70',
                    status: 'Finished',
                    Time: { millis: '5529143', time: '1:32:09.143' },
                    FastestLap: {
                      rank: '5',
                      lap: '55',
                      Time: { time: '1:16.561' },
                      AverageSpeed: { units: 'kph', speed: '205.060' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '8',
                url: 'http://en.wikipedia.org/wiki/2013_British_Grand_Prix',
                raceName: 'British Grand Prix',
                Circuit: {
                  circuitId: 'silverstone',
                  url: 'http://en.wikipedia.org/wiki/Silverstone_Circuit',
                  circuitName: 'Silverstone Circuit',
                  Location: {
                    lat: '52.0786',
                    long: '-1.01694',
                    locality: 'Silverstone',
                    country: 'UK',
                  },
                },
                date: '2013-06-30',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '9',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'rosberg',
                      permanentNumber: '6',
                      code: 'ROS',
                      url: 'http://en.wikipedia.org/wiki/Nico_Rosberg',
                      givenName: 'Nico',
                      familyName: 'Rosberg',
                      dateOfBirth: '1985-06-27',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'mercedes',
                      url: 'http://en.wikipedia.org/wiki/Mercedes-Benz_in_Formula_One',
                      name: 'Mercedes',
                      nationality: 'German',
                    },
                    grid: '2',
                    laps: '52',
                    status: 'Finished',
                    Time: { millis: '5579456', time: '1:32:59.456' },
                    FastestLap: {
                      rank: '2',
                      lap: '50',
                      Time: { time: '1:33.531' },
                      AverageSpeed: { units: 'kph', speed: '226.744' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '9',
                url: 'http://en.wikipedia.org/wiki/2013_German_Grand_Prix',
                raceName: 'German Grand Prix',
                Circuit: {
                  circuitId: 'nurburgring',
                  url: 'http://en.wikipedia.org/wiki/N%C3%BCrburgring',
                  circuitName: 'Nürburgring',
                  Location: {
                    lat: '50.3356',
                    long: '6.9475',
                    locality: 'Nürburg',
                    country: 'Germany',
                  },
                },
                date: '2013-07-07',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '2',
                    laps: '60',
                    status: 'Finished',
                    Time: { millis: '6074711', time: '1:41:14.711' },
                    FastestLap: {
                      rank: '5',
                      lap: '59',
                      Time: { time: '1:34.164' },
                      AverageSpeed: { units: 'kph', speed: '196.814' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '10',
                url: 'http://en.wikipedia.org/wiki/2013_Hungarian_Grand_Prix',
                raceName: 'Hungarian Grand Prix',
                Circuit: {
                  circuitId: 'hungaroring',
                  url: 'http://en.wikipedia.org/wiki/Hungaroring',
                  circuitName: 'Hungaroring',
                  Location: {
                    lat: '47.5789',
                    long: '19.2486',
                    locality: 'Budapest',
                    country: 'Hungary',
                  },
                },
                date: '2013-07-28',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '10',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'hamilton',
                      permanentNumber: '44',
                      code: 'HAM',
                      url: 'http://en.wikipedia.org/wiki/Lewis_Hamilton',
                      givenName: 'Lewis',
                      familyName: 'Hamilton',
                      dateOfBirth: '1985-01-07',
                      nationality: 'British',
                    },
                    Constructor: {
                      constructorId: 'mercedes',
                      url: 'http://en.wikipedia.org/wiki/Mercedes-Benz_in_Formula_One',
                      name: 'Mercedes',
                      nationality: 'German',
                    },
                    grid: '1',
                    laps: '70',
                    status: 'Finished',
                    Time: { millis: '6149445', time: '1:42:29.445' },
                    FastestLap: {
                      rank: '3',
                      lap: '69',
                      Time: { time: '1:24.647' },
                      AverageSpeed: { units: 'kph', speed: '186.322' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '11',
                url: 'http://en.wikipedia.org/wiki/2013_Belgian_Grand_Prix',
                raceName: 'Belgian Grand Prix',
                Circuit: {
                  circuitId: 'spa',
                  url: 'http://en.wikipedia.org/wiki/Circuit_de_Spa-Francorchamps',
                  circuitName: 'Circuit de Spa-Francorchamps',
                  Location: {
                    lat: '50.4372',
                    long: '5.97139',
                    locality: 'Spa',
                    country: 'Belgium',
                  },
                },
                date: '2013-08-25',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '2',
                    laps: '44',
                    status: 'Finished',
                    Time: { millis: '5022196', time: '1:23:42.196' },
                    FastestLap: {
                      rank: '1',
                      lap: '40',
                      Time: { time: '1:50.756' },
                      AverageSpeed: { units: 'kph', speed: '227.657' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '12',
                url: 'http://en.wikipedia.org/wiki/2013_Italian_Grand_Prix',
                raceName: 'Italian Grand Prix',
                Circuit: {
                  circuitId: 'monza',
                  url: 'http://en.wikipedia.org/wiki/Autodromo_Nazionale_Monza',
                  circuitName: 'Autodromo Nazionale di Monza',
                  Location: {
                    lat: '45.6156',
                    long: '9.28111',
                    locality: 'Monza',
                    country: 'Italy',
                  },
                },
                date: '2013-09-08',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '53',
                    status: 'Finished',
                    Time: { millis: '4713352', time: '1:18:33.352' },
                    FastestLap: {
                      rank: '12',
                      lap: '52',
                      Time: { time: '1:27.190' },
                      AverageSpeed: { units: 'kph', speed: '239.187' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '13',
                url: 'http://en.wikipedia.org/wiki/2013_Singapore_Grand_Prix',
                raceName: 'Singapore Grand Prix',
                Circuit: {
                  circuitId: 'marina_bay',
                  url: 'http://en.wikipedia.org/wiki/Marina_Bay_Street_Circuit',
                  circuitName: 'Marina Bay Street Circuit',
                  Location: {
                    lat: '1.2914',
                    long: '103.864',
                    locality: 'Marina Bay',
                    country: 'Singapore',
                  },
                },
                date: '2013-09-22',
                time: '12:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '61',
                    status: 'Finished',
                    Time: { millis: '7153132', time: '1:59:13.132' },
                    FastestLap: {
                      rank: '1',
                      lap: '46',
                      Time: { time: '1:48.574' },
                      AverageSpeed: { units: 'kph', speed: '167.940' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '14',
                url: 'http://en.wikipedia.org/wiki/2013_Korean_Grand_Prix',
                raceName: 'Korean Grand Prix',
                Circuit: {
                  circuitId: 'yeongam',
                  url: 'http://en.wikipedia.org/wiki/Korean_International_Circuit',
                  circuitName: 'Korean International Circuit',
                  Location: {
                    lat: '34.7333',
                    long: '126.417',
                    locality: 'Yeongam County',
                    country: 'Korea',
                  },
                },
                date: '2013-10-06',
                time: '06:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '55',
                    status: 'Finished',
                    Time: { millis: '6193701', time: '1:43:13.701' },
                    FastestLap: {
                      rank: '1',
                      lap: '53',
                      Time: { time: '1:41.380' },
                      AverageSpeed: { units: 'kph', speed: '199.388' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '15',
                url: 'http://en.wikipedia.org/wiki/2013_Japanese_Grand_Prix',
                raceName: 'Japanese Grand Prix',
                Circuit: {
                  circuitId: 'suzuka',
                  url: 'http://en.wikipedia.org/wiki/Suzuka_Circuit',
                  circuitName: 'Suzuka Circuit',
                  Location: {
                    lat: '34.8431',
                    long: '136.541',
                    locality: 'Suzuka',
                    country: 'Japan',
                  },
                },
                date: '2013-10-13',
                time: '06:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '2',
                    laps: '53',
                    status: 'Finished',
                    Time: { millis: '5209301', time: '1:26:49.301' },
                    FastestLap: {
                      rank: '4',
                      lap: '39',
                      Time: { time: '1:35.317' },
                      AverageSpeed: { units: 'kph', speed: '219.322' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '16',
                url: 'http://en.wikipedia.org/wiki/2013_Indian_Grand_Prix',
                raceName: 'Indian Grand Prix',
                Circuit: {
                  circuitId: 'buddh',
                  url: 'http://en.wikipedia.org/wiki/Buddh_International_Circuit',
                  circuitName: 'Buddh International Circuit',
                  Location: {
                    lat: '28.3487',
                    long: '77.5331',
                    locality: 'Uttar Pradesh',
                    country: 'India',
                  },
                },
                date: '2013-10-27',
                time: '09:30:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '60',
                    status: 'Finished',
                    Time: { millis: '5472187', time: '1:31:12.187' },
                    FastestLap: {
                      rank: '2',
                      lap: '54',
                      Time: { time: '1:28.116' },
                      AverageSpeed: { units: 'kph', speed: '209.383' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '17',
                url: 'http://en.wikipedia.org/wiki/2013_Abu_Dhabi_Grand_Prix',
                raceName: 'Abu Dhabi Grand Prix',
                Circuit: {
                  circuitId: 'yas_marina',
                  url: 'http://en.wikipedia.org/wiki/Yas_Marina_Circuit',
                  circuitName: 'Yas Marina Circuit',
                  Location: {
                    lat: '24.4672',
                    long: '54.6031',
                    locality: 'Abu Dhabi',
                    country: 'UAE',
                  },
                },
                date: '2013-11-03',
                time: '13:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '2',
                    laps: '55',
                    status: 'Finished',
                    Time: { millis: '5886106', time: '1:38:06.106' },
                    FastestLap: {
                      rank: '2',
                      lap: '51',
                      Time: { time: '1:43.893' },
                      AverageSpeed: { units: 'kph', speed: '192.451' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '18',
                url: 'http://en.wikipedia.org/wiki/2013_United_States_Grand_Prix',
                raceName: 'United States Grand Prix',
                Circuit: {
                  circuitId: 'americas',
                  url: 'http://en.wikipedia.org/wiki/Circuit_of_the_Americas',
                  circuitName: 'Circuit of the Americas',
                  Location: {
                    lat: '30.1328',
                    long: '-97.6411',
                    locality: 'Austin',
                    country: 'USA',
                  },
                },
                date: '2013-11-17',
                time: '19:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '56',
                    status: 'Finished',
                    Time: { millis: '5957148', time: '1:39:17.148' },
                    FastestLap: {
                      rank: '1',
                      lap: '54',
                      Time: { time: '1:39.856' },
                      AverageSpeed: { units: 'kph', speed: '198.754' },
                    },
                  },
                ],
              },
              {
                season: '2013',
                round: '19',
                url: 'http://en.wikipedia.org/wiki/2013_Brazilian_Grand_Prix',
                raceName: 'Brazilian Grand Prix',
                Circuit: {
                  circuitId: 'interlagos',
                  url: 'http://en.wikipedia.org/wiki/Aut%C3%B3dromo_Jos%C3%A9_Carlos_Pace',
                  circuitName: 'Autódromo José Carlos Pace',
                  Location: {
                    lat: '-23.7036',
                    long: '-46.6997',
                    locality: 'São Paulo',
                    country: 'Brazil',
                  },
                },
                date: '2013-11-24',
                time: '16:00:00Z',
                Results: [
                  {
                    number: '1',
                    position: '1',
                    positionText: '1',
                    points: '25',
                    Driver: {
                      driverId: 'vettel',
                      permanentNumber: '5',
                      code: 'VET',
                      url: 'http://en.wikipedia.org/wiki/Sebastian_Vettel',
                      givenName: 'Sebastian',
                      familyName: 'Vettel',
                      dateOfBirth: '1987-07-03',
                      nationality: 'German',
                    },
                    Constructor: {
                      constructorId: 'red_bull',
                      url: 'http://en.wikipedia.org/wiki/Red_Bull_Racing',
                      name: 'Red Bull',
                      nationality: 'Austrian',
                    },
                    grid: '1',
                    laps: '71',
                    status: 'Finished',
                    Time: { millis: '5556300', time: '1:32:36.300' },
                    FastestLap: {
                      rank: '3',
                      lap: '51',
                      Time: { time: '1:15.624' },
                      AverageSpeed: { units: 'kph', speed: '205.125' },
                    },
                  },
                ],
              },
            ],
          },
        },
      }
      jest.spyOn(ErgastService, 'getSeasonResults').mockResolvedValue(mockData);

      const resp = (await Promise.all([
        F1Service.getRaceWinnersBySeason(2013),
        F1Service.getRaceWinnersBySeason(2013),
        F1Service.getRaceWinnersBySeason(2013),
        F1Service.getRaceWinnersBySeason(2013),
        F1Service.getRaceWinnersBySeason(2013),
      ]))[0];

      expect(resp?.year).toBe(Number(mockData.MRData.RaceTable.season));
    } catch (e) {
      console.error(e);
    }
  });
});

afterAll(async () => {
  await prisma.$disconnect();
});
